...
constructor() {
  ...
  ngOnInit() {
    Hammer.on(window, "load resize scroll", function(ev) {
      ...
      Hammer.on(window, "load resize scroll", function(evt) {
        // var sliderEl = document.querySelector( '.slider' );
        let sliderElt = document.getElementById( 'sliderTwo' );
        let slideCount = 2;
        let activeSlide = 0; // NEW: the current slide # (0 = first)
        let sliderManager = new Hammer.Manager( sliderElt );
        sliderManager.add( new Hammer.Pan({ threshold: 0, pointers: 0 }) );
        sliderManager.on( 'pan', function( e ) {
          let percentage = 100 / slideCount * e.deltaX / window.innerWidth;
          let transformPercentage = percentage - 100 / slideCount * activeSlide; // NEW
          sliderElt.style.transform = 'translateX( ' + transformPercentage + '% )';
          if( e.isFinal ) { // NEW: this only runs on event end
            if( percentage < 0 )
              goToSlideTwo( activeSlide + 1 );
            else if( percentage > 0 )
              goToSlideTwo( activeSlide - 1 );
            else
              goToSlideTwo( activeSlide );
          }
        });

        // NEW: function that changes the slide
        let goToSlideTwo = function( number ) {
          if( number < 0 )
            activeSlide = 0;
          else if( number > slideCount - 1 )
            activeSlide = slideCount - 1
            ...
            let percentage = -( 100 / slideCount ) * activeSlide;
            sliderElt.style.transform = 'translateX( ' + percentage + '% )';
        };
        });
  }
